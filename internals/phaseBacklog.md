...
2. Validate hosted replica-set initialization, including correct `MONGO_REPLICA_KEY` and `WHATSAPP_GATEWAY_TOKEN` secret wiring, `/ready`, TLS issuance, and branch-based deploy behavior on staging before touching production.
3. Pair the real CJ Laundry number through the new admin WhatsApp status page on staging and confirm the auth volume survives gateway restarts.
