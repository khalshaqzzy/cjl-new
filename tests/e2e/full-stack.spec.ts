...
test("admin and public frontends stay fully integrated through the backend", async ({ browser, page }) => {
  ...
  const employeeUsername = `employee-${uniqueSuffix}`
  const employeePassword = `employee-pass-${uniqueSuffix}`
  await page.getByTestId("settings-employee-username").fill(employeeUsername)
  await page.getByTestId("settings-employee-password").fill(employeePassword)
  ...
  await employeePage.getByTestId("admin-login-password").fill(employeePassword)
  await employeePage.getByTestId("admin-login-submit").click()
  await expect(employeePage).toHaveURL(/\/admin$/)
  await expect(employeePage.getByText("Pengaturan")).not.toBeVisible()
  await expect(employeePage.getByText("Kontrol Mesin")).not.toBeVisible()
  await expect(employeePage.getByRole("button", { name: "Hari Ini" })).toBeVisible()
  await expect(employeePage.getByRole("button", { name: "Minggu Ini" })).not.toBeVisible()
  await expect(employeePage.getByRole("button", { name: "Bulan Ini" })).not.toBeVisible()
  await employeePage.goto("http://127.0.0.1:3101/admin/laundry-aktif")
  await expect(employeePage.getByRole("tab", { name: "Aktif" })).toBeVisible()
  await expect(employeePage.getByRole("tab", { name: "Hari Ini" })).toBeVisible()
  await expect(employeePage.getByRole("tab", { name: "History" })).not.toBeVisible()
  await employeePage.goto("http://127.0.0.1:3101/admin/settings")
  await expect(employeePage).toHaveURL(/\/admin$/)
  ...
  await page.getByRole("button", { name: "Detail Pelanggan" }).click()
  await page.getByRole("tab", { name: /Riwayat Poin/ }).click()
  await page.getByRole("button", { name: "Tambah Manual" }).click()
  await page.getByTestId("manual-points-input").fill("5")
  await page.getByTestId("manual-points-sign-minus").click()
  await expect(page.getByTestId("manual-points-input")).toHaveValue("-5")
  await expect(page.getByText("15")).toBeVisible()
  await page.getByPlaceholder("Contoh: Kompensasi keterlambatan, bonus loyalitas...").fill("Koreksi saldo E2E")
  await page.getByTestId("manual-points-submit").click()
  await expect(page.getByText("Poin manual berhasil dikurangi.")).toBeVisible()
  ...
  await expect(page.getByText("Setiap redeem memberi diskon Rp 10.000 dan mengurangi 1 kesempatan poin dari order ini.")).toBeVisible()
  ...
  await page.getByTestId(`customer-result-${customer!._id}`).click()
  await page.getByTestId("pos-next-to-services").click()
  await page.getByTestId("pos-weight-input").fill("2")
  await page.getByTestId("service-plus-wash_dry_fold_package").click()
  await page.getByTestId("pos-redeem-plus").click()
  await expect(page.getByText("Setiap redeem memberi diskon Rp 10.000 dan mengurangi 1 kesempatan poin dari order ini.")).toBeVisible()
  await page.getByTestId("pos-open-summary").click()
  const packageRedeemSummaryDialog = page.getByRole("dialog")
  await expect(packageRedeemSummaryDialog.getByText("1x Diskon Reward")).toBeVisible()
  await expect(packageRedeemSummaryDialog.getByText("1 poin")).toBeVisible()
  await page.getByTestId("pos-confirm-order").click()

  const packageRedeemOrderCode = (await page.getByTestId("pos-success-order-code").innerText()).trim()
  const { order: packageRedeemOrder } = await waitForOrderRecord(packageRedeemOrderCode)
  expect(packageRedeemOrder.items.some((item: { serviceCode: string }) => item.serviceCode === "wash_dry_fold_package")).toBeTruthy()
  expect(packageRedeemOrder.discount).toBe(10000)
  expect(packageRedeemOrder.earnedStamps).toBe(0)
  expect(packageRedeemOrder.redeemedPoints).toBe(10)
  ...
  await page.getByTestId("whatsapp-provider-toggle").click()
  await expect(page.getByText("Phone Number ID")).toBeVisible()
  const mobileThreadListItem = page.getByTestId("whatsapp-thread-wa:62" + customerPhone.slice(1))
  const mobileListViewport = page.viewportSize()!
  const mobileListItemBox = await mobileThreadListItem.boundingBox()
  expect(mobileListItemBox!.x + mobileListItemBox!.width).toBeLessThanOrEqual(mobileListViewport.width + 1)
  await mobileThreadListItem.click()
  ...
  const mobileHeader = page.getByTestId("whatsapp-thread-header")
  const mobileComposer = page.getByTestId("whatsapp-thread-composer")
  const mobileTimeline = page.getByTestId("whatsapp-thread-timeline")
  ...
  expect(mobileBubbleBox!.x + mobileBubbleBox!.width).toBeLessThanOrEqual(mobileViewport.width + 1)
  const mobileHeaderBefore = await mobileHeader.boundingBox()
  const mobileComposerBefore = await mobileComposer.boundingBox()
  const mobilePageScrollBeforeTimelineWheel = await page.evaluate(() => window.scrollY)
  await mobileTimeline.hover()
  await page.mouse.wheel(0, 1400)
  await expect
    .poll(async () =>
      mobileTimeline.locator('[data-slot="scroll-area-viewport"]').evaluate((node) => node.scrollTop)
    )
    .toBeGreaterThan(0)
  await expect
    .poll(async () => page.evaluate(() => window.scrollY))
    .toBe(mobilePageScrollBeforeTimelineWheel)
